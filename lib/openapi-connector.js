  ...
  new SpecResolver().resolve(
    ...
    async function(err, api) {
      ...
      try {
        ...
        if (req.spec.openapi) {
          let baseURL;
          if (self.settings.spec && typeof self.settings.spec === 'string') {
            if (self.settings.spec.includes('http') || self.settings.spec.includes('https')) {
              const baseURLObject = new URL(self.settings.spec);
          ...
          if (!req.spec.servers || !req.spec.servers.length) {
            ...
            req.spec.servers.forEach((server, index) => {
              if (!server.url.includes('http')&& !server.url.includes('https')) {
                req.spec.servers[index].url = `${baseURL}/${req.spec.servers[index].url}`;
              }
            });
          }
        }
